...
- **Settings** — view all settings and live-edit idle-timeout / API key / **tiered-KV cache** (hot-entry capacity + cold SSD dir, applied to new model loads) / a **HuggingFace mirror endpoint** / **generation defaults** (default temperature · top_p · top_k · max_tokens, applied only when a request omits them — a request's own value always wins)
