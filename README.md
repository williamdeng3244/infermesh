...
config. **M5** — real vLLM GPU backend (verified on an RTX 5070, Blackwell) + latency/throughput charts. **M6** — light/dark theme + real multi-model LRU eviction on the GPU. **M7** — benchmark suite (latency percentiles · TTFT · throughput). **M8** — hosted-model proxy backend: register OpenAI / Anthropic / OpenRouter / any OpenAI-compatible endpoint via `--providers`, so remote models join the same pool, dashboard, and OpenAI+Anthropic gateway as local vLLM models. **M9** — Claude Code hardening: SSE keep-alives during long prefill, `response_format` parity on the hosted path, and cross-platform background service management (`start` / `stop` / `restart` / `status`). **M10** — in-process **Transformers backend** (`AutoModelForCausalLM` on CUDA / CPU / MPS), the bring-your-own-accelerator path; decoding raw text locally also activates the model-family tool-call parsers (Qwen-XML / Hermes / Llama-bracket / Gemma4) — **verified generating on an RTX 5070**. **M11** — observability: a **Devices** tab (enumerate NVIDIA / AMD / CPU + a per-model GPU picker) and **Metrics + Benchmark history** persisted to `~/.infermesh` so past tests survive a restart. **M12** — comprehensive benchmark: prefill (PP) + decode (TG) tok/s, TTFT, TPOT, E2E percentiles, peak GPU memory, and `same` vs `different` prompt modes (prefix-cache effect), with a single-request profile and copy-to-clipboard. **M13** — **Model Downloader**: search HuggingFace from the dashboard and one-click download a repo into the model dir (background, with progress), auto-registered into the pool when finished — sort by **trending / downloads / likes / recently-updated**, filter by **task** (text-gen / vision / embedding), with a popular-models default list. **M14** — **MCP server**: `infermesh mcp` exposes 14 tools (list/load/unload/pin models, run_benchmark, chat, devices, metrics, search/download HF) over stdio so Claude Code and other agents can drive infermesh and run tests themselves. **M15** — **vision-language models**: OpenAI/Anthropic image inputs (base64 / URL / file) flow through a new `InternalMessage.images` to a Transformers vision path (`AutoModelForImageTextToText`) — **verified on an RTX 5070** (SmolVLM correctly read a test image). **M16** — **micro-batching** for the Transformers backend: concurrent requests coalesce into one batched `generate` (opt-in via `extra["micro_batch"]`) — verified batching 4 concurrent requests on an RTX 5070 (vLLM does true token-level batching itself). **M17** — **tiered KV cache** for the Transformers backend: a prefix-keyed hot tier (RAM, LRU) + cold tier (SSD) that spills on overflow and restores on a hit — survives a restart; opt-in via `extra["prefix_kv"]`. **M18** — expandable per-run benchmark history (click a row for its full breakdown). **M19** — aggregate request stats with a **Session / All-Time** toggle (requests, tokens served, prompt/completion/cached tokens, cache efficiency %, prefill/generation tok/s), **per-model or global**; all-time persisted across restarts, plus **uptime** and **preflight-rejection-by-reason** counts. **M20** — server-side **generation defaults** (temperature / top_p / top_k / max_tokens) filled in when a request omits them, editable from Settings. **M21** — editable **startup settings** (host / port / model-dir / backend / max-memory) from the dashboard, each saved immediately and applied by a one-click **Restart** (in-process `os.execv` re-exec that keeps the PID — so a managed pidfile stays valid — and reloads `settings.json`). **M22** — dashboard **EN ⇄ 中文** language toggle: a comprehensive UI translation (nav, headers, labels, tables, hints, buttons, dynamic stat labels) driven by a `data-i18n` walker + an English-keyed dictionary, with the choice persisted; units (`tok/s`, `ms`, `%`) and proper nouns stay as-is. **M23** — **admission control**: a control-plane concurrency gate that actually enforces `max_concurrent_requests`, queues the excess (FIFO, bounded), and returns **503** when the queue is full or the wait times out — live-tunable from Settings, surfaced as authoritative active/queued counts in `/api/status` and the Metrics live bar (verified: 30 concurrent against cap=1/queue=1 → 3×200 + 27×503). **M24** — **per-model generation overrides**: a model can pin its own temperature / top_p / top_k / max_tokens (precedence: request > per-model > global > fallback) plus a `max_context_window` that rejects over-long prompts (approximate, char-based — the control plane has no tokenizer), persisted to `~/.infermesh/model_settings.json` and editable from the Models tab. **M25** — **ModelScope** as a second download source: pull a model by ID from ModelScope Hub (`pip install 'infermesh[modelscope]'`) alongside HuggingFace, behind the same lazy-import seam so the control plane still installs without it — degrades to a clear "not installed" job error when the SDK is absent. 117 tests green on the mock backend (no GPU).
...
- **Metrics** — aggregate **Session / All-Time** stats (requests, tokens served, cache efficiency, prefill/gen tok/s, uptime, rejections-by-reason), **per-model or global**, organized into **Overview / Per-model / Charts / Rejections** sub-tabs with a **sortable per-model table**, a **live status bar** (loaded / active / queue / recent tok/s), **Copy** + **Export CSV**, a **token-composition bar** + **cache-efficiency** gauge, derived metrics (avg tokens/req, success rate), and latency/throughput charts with **p95 / peak** and a **5m / 1h / all** time-range (canvas, no chart lib)
...
**Micro-batching.** Set `extra={"micro_batch": 8}` and concurrent requests are
coalesced (within `batch_window`, default 10 ms) into one left-padded batched
`generate` — a throughput win under concurrent load. Verified on an RTX 5070:
4 concurrent requests batched into a single pass. (vLLM does true token-level
continuous batching in its own process; this is the in-process equivalent.)

**Tiered KV cache.** Set `extra={"prefix_kv": 8}` (hot capacity; optional
`kv_cold_dir`) and the backend builds a `TieredKVCache`: a prefix-keyed hot tier in
RAM (LRU) backed by a cold tier on SSD — overflow spills to disk and a later hit
restores it, so a cached prefix survives eviction and even a restart (cache stats
appear in `/api/status`). This is the hot/cold store (the oMLX-style mechanism);
wiring block-level prefix reuse into `generate` is model-specific and layered on top.
