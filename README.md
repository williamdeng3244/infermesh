...
- **Metrics** — aggregate **Session / All-Time** stats (requests, tokens served, cache efficiency, prefill/gen tok/s) + latency + throughput sparkline charts (canvas, no chart lib)
...
89 tests, all green with `MockEchoBackend` — **no GPU, no model, and vllm/torch not
