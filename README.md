...
- **Metrics** — aggregate **Session / All-Time** stats (requests, tokens served, cache efficiency, prefill/gen tok/s), **per-model or global**, + latency + throughput sparkline charts (canvas, no chart lib)
...
90 tests, all green with `MockEchoBackend` — **no GPU, no model, and vllm/torch not
